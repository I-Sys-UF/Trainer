...
- Train-2 を動かす
  - [資料](./Train-2/README.md)を見ながら進める
  - if 文を使う練習
  - まずは `if(true)` と `if(false)` を動かす
  - スイッチを読み取り，押されているときに LED を光らせてみる
  - 逆もやってみよう
- Train-3 を動かす
  - [資料](./Train-3/README.md)を見ながら進める
  - 関数を作る
  - 任意の f(x) の式を実装
- Train-4 を動かす
  - [資料](./Train-4/README.md)を見ながら進める
  - シフトレジスタにデータを送る練習
...
オリジナルプログラムを作ったら，新しいリポジトリを立ててそこで管理してみよう  
ちなみに，部員であれば自由にリポジトリを立ててよい
